...
userinfoInput <- userinfoInput %>%
 ...
 mutate(allergiesinput = pmin(allergiesinput, housholdnumberinput - 1)) |>
  ...
  mutate(experienceinput = rpois(n = n(), lambda = 2)) |>
...
final_data |>
  filter(n_values >= 19900)
  
  ggplot(aes(x = n_values, y = totalownstate/n_values, color = state)) + 
  geom_line() + 
  theme_bw() +
  labs(x = "Simulation Sample Size", y = "Ownership Prepared Percentage",
       color = "States")
