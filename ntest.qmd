...
nlargenum <- function(n) {

california_homeownership_rate <- 0.546
...
userinfoInput <- userinfoInput %>%
  ...
  mutate(salaryinput = case_when(
    state == "California" ~ sample(seq(62525 * 0.10, 62525 * 1.1, by = 0.1), size = n(), replace = TRUE),
    state == "West Virginia" ~ sample(seq(40539 * 0.10, 40539 * 1.1, by = 0.1), size = n(), replace = TRUE),
    state == "New York" ~ sample(seq(56897 * 0.10, 56897 * 1.1, by = 0.1), size = n(), replace = TRUE)
  )) |>
  ungroup() 
...
resultdf <- PWBindexDF |>
  ...
  filter(totalcount >= 5) |>
  select(state, ID) |>
  summarise(totalownstate = n(), .by = state) |>
  mutate(n_values = n)
...
for (n in seq(1, 10000, by = 100)) {
  ...
  all_data[[as.character(n)]] <- nlargenum(n)
}

final_data <- bind_rows(all_data, .id = "n_val")

final_data |>
  ggplot(aes(x = n_values, y = totalownstate/n_values, color = state)) + 
  geom_line()
...
set.seed(1003) # For reproducibility

# Define the range of n
n_values <- seq(1, 100000, by = 1)

# Simulate coin flips for both coins and compute the cumulative averages
results <- tibble(
  Trials = n_values,
  GreenAverage = sapply(n_values, function(n) {
    mean(sample(c(0, 1), size = n, replace = TRUE, prob = c(0.5, 0.5))) # 0 for tails, 1 for heads for green coin
  }),
  RedAverage = sapply(n_values, function(n) {
    mean(sample(c(0, 1), size = n, replace = TRUE, prob = c(0.47, 0.53))) # 0 for tails, 1 for heads for red coin
  })
)

# Plot the results
ggplot(results, aes(x = Trials)) +
  geom_line(aes(y = GreenAverage, color = "Green")) +
  geom_line(aes(y = RedAverage, color = "Red")) +
  geom_hline(yintercept = 0.5, linetype = "dashed", color = "green") +
  geom_hline(yintercept = 0.53, linetype = "dashed", color = "red") +
  labs(title = "Demonstration of the Law of Large Numbers with Two Coins",
       x = "Number of Trials",
